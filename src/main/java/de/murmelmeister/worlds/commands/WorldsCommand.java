...
public final class WorldsCommand extends CommandManager {
    public WorldsCommand(Worlds plugin) {
        super(plugin);
    }

    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!(config.getBoolean(Configs.COMMAND_ENABLE_WORLDS_COMMAND))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_DISABLE));
            return true;
        }

        if (!(sender.hasPermission(permission.getPermission(Permissions.COMMAND_WORLDS_COMMAND)))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_NO_PERMISSION));
            return true;
        }
        ...
        if (args.length == 0 || (args.length == 1 && !args[0].equalsIgnoreCase("test")) || (args.length == 2 && (args[0].equalsIgnoreCase("create")))) { // TODO: Gives args errors?
            sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_SYNTAX_COMMAND));
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 2 && !args[0].equalsIgnoreCase("gamerule")) {
            String lastWord = args[args.length - 1];
            for (String world : worldManager.getWorldList()) {
    ...
    private void worldsCreateCommand(CommandSender sender, String[] args) {
        if (!(config.getBoolean(Configs.COMMAND_ENABLE_WORLDS_CREATE))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_DISABLE));
            return;
        }

        if (!(sender.hasPermission(permission.getPermission(Permissions.COMMAND_WORLDS_CREATE)))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_NO_PERMISSION));
            return;
        }
        ...
        if (sender.getServer().getWorld(worldName) != null) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_DOES_EXIST).replace("[WORLD]", worldName));
            return;
        }
        ...
        switch (worldEnvironment) {
            case "normal":
                ...
                sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_SUCCESSFUL_CREATE).replace("[WORLD]", worldName));
                break;
            case "nether":
            ...
            case "end":
                ...
                break;
            default:
                sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_SYNTAX_CREATE));
                break;
        }
    }

    private void worldsDeleteCommand(CommandSender sender, String[] args) {
        if (!(config.getBoolean(Configs.COMMAND_ENABLE_WORLDS_DELETE))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_DISABLE));
            return;
        }

        if (!(sender.hasPermission(permission.getPermission(Permissions.COMMAND_WORLDS_DELETE)))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_NO_PERMISSION));
            return;
        }
        ...
        if (world == null) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_DOES_NOT_EXIST).replace("[WORLD]", args[1]));
            return;
        }
        ...
        if (player != null)
            if (player.getWorld().equals(world))
                player.kick(Component.text(message.getMessage(Messages.COMMAND_WORLDS_DELETE_KICKED_PLAYER)));

        worldManager.deleteWorld(world.getName());
        sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_SUCCESSFUL_DELETE).replace("[WORLD]", world.getName()));
    }

    private void worldsTeleportCommand(CommandSender sender, String[] args) {
        if (!(config.getBoolean(Configs.COMMAND_ENABLE_WORLDS_TELEPORT))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_DISABLE));
            return;
        }

        if (!(sender.hasPermission(permission.getPermission(Permissions.COMMAND_WORLDS_TELEPORT)))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_NO_PERMISSION));
            return;
        }
        ...
        if (player == null) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_NO_CONSOLE));
            return;
        }

        player.teleport(world.getSpawnLocation());
        sendMessage(player, message.getMessage(Messages.COMMAND_WORLDS_TELEPORT).replace("[WORLD]", world.getName()));
    }

    private void worldsImportCommand(CommandSender sender, String[] args) {
        if (!(config.getBoolean(Configs.COMMAND_ENABLE_WORLDS_IMPORT))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_DISABLE));
            return;
        }

        if (!(sender.hasPermission(permission.getPermission(Permissions.COMMAND_WORLDS_IMPORT)))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_NO_PERMISSION));
            return;
        }
    ...
    private void worldsGameRuleCommand(CommandSender sender, String[] args) {
        if (!(config.getBoolean(Configs.COMMAND_ENABLE_WORLDS_GAMERULE))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_DISABLE));
            return;
        }

        if (!(sender.hasPermission(permission.getPermission(Permissions.COMMAND_WORLDS_GAMERULE)))) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_NO_PERMISSION));
            return;
        }
        ...
        if (world == null) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_DOES_NOT_EXIST).replace("[WORLD]", args[2]));
            return;
        }
        ...
        if (gameRule == null) {
            sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_GAMERULE_WRONG_GAMERULE).replace("[GAMERULE]", args[3]));
            return;
        }
        ...
        switch (args[1]) {
            case "get":
                sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_GAMERULE_GET).replace("[GAMERULE]", gameRule.getName()).replace("[VALUE]", worldManager.getSting(path)));
                break;
            case "set":
                Object value = args[4];
                worldManager.set(path, value);
                world.setGameRuleValue(gameRule.getName(), value.toString()); // deprecated Method
                sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_GAMERULE_SET).replace("[GAMERULE]", gameRule.getName()).replace("[VALUE]", value.toString()));
                break;
            default:
                sendMessage(sender, message.getMessage(Messages.COMMAND_WORLDS_SYNTAX_GAMERULE));
                break;
        }
